...
namespace Classroom.PUN.UI
{
    ...
    public class MyRoomChat : MyChat, IOnEventCallback, IInRoomCallbacks, IMatchmakingCallbacks
    {
        public static MyRoomChat Instance => mInst;
        private static MyRoomChat mInst;
        ...
        private void FloatingMessage(string message, Color color, Player player)
        {
            if (player?.TagObject != null)
            {
                ...
                bool isChild = (bool)VariablesManager.GetLocal(go, "isChild", false);
                if (!isChild)
                {
                    MyDebug.Log("Player:" + player.ToStringFull() + " isChild:" + isChild);
                    FloatingMessageManager.Show(
                        message, color,
                        go.transform, floatingMessageOffset + new Vector3(0, 0.5f, 0), floatingMessageTime
                    );
                }
                else
                {
